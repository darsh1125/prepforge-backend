...
async function flashcardMutation(req: Request, res: Response, action: "update" | "create" | "delete" | "pin"): Promise<void> {
  ...
  else { const index = flashcards.findIndex((card) => card.id === flashcardId); if (index < 0) throw new AppError("NOT_FOUND", "Flashcard not found", 404, { expose: true }); if (action === "delete") { const [removed] = flashcards.splice(index, 1); const internalId = metadata[index]!.internalId; record.set("editorMetadata", { ...data, deletedFlashcardIds: [...((data.deletedFlashcardIds as string[] | undefined) ?? []), removed!.id] }); metadata.splice(index, 1); await PracticeState.deleteOne({ kitId: record._id, ownerId: record.ownerId, flashcardInternalId: internalId }); } else if (action === "pin") metadata[index]!.pinned = body.pinned ?? false; else { validReferences(body.requirement_ids ?? flashcards[index]!.requirement_ids, role); flashcards[index] = flashcardSchema.parse({ ...flashcards[index], ...body, expectedRevision: undefined }); metadata[index]!.edited = true; } }
